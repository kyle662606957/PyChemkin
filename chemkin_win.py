...
CHEMKIN_SCRIPT_PREAMBLE = """rem #!/bin/sh -v

rem # Define Chemkin running environment
CALL "{0}"

""".format(os.path.join(CHEMKIN_DIR, 'bin', 'run_chemkin_env_setup.bat'))
...
class ChemkinJob(object):
    ...
    def preprocess(self):
        ...
        scriptFile = os.path.join(self.tempDir, 'RUNJOB_CKPreProcess.bat')
        with open(scriptFile, 'w') as stream:
            stream.write(CHEMKIN_SCRIPT_PREAMBLE)
            stream.write('rem # Run the Chemkin preprocessor\n')
            stream.write('CALL "{0}" -i {1} -o {2}_gas.out -c {2}_gas.asc\n\n'.format(
        ...
        process = subprocess.Popen( scriptFile, cwd=self.tempDir, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def run(self, jobParams, model, pro=True):
        ...
        scriptFile = os.path.join(self.tempDir, 'RUNJOB_CKRunProcessor.bat')
        ...
        with open(scriptFile, 'w') as stream:
            stream.write(CHEMKIN_SCRIPT_PREAMBLE)
            stream.write('rem # Delete any intermediate files left over from a previous run\n')
            stream.write('rd /s /q {0}\n\n'.format(self.dataZipFile))
            stream.write('rd /s /q  {0}\n\n'.format(dtdFileDest))
            stream.write('rem # Copy files to working directory\n')
            stream.write('COPY  "{0}"  "{1}"\n'.format(dtdFileSrc, dtdFileDest))
            stream.write('rem # Run the job\n')
            if pro:
                stream.write('rem CHEMKIN_MODE=Pro\n')
                stream.write('SET CHEMKIN_MODE=Pro\n')
                stream.write('CALL "{0}" -i {1} -o {2} -x {3} Pro -m {4} -c {5}\n'.format(model, self.inputFile, self.outputFile, self.dataZipFile, self.monFile, self.ascFile))
            else:
                stream.write('CALL "{0}" -i {1} -o {2} -x {3} -m {4} -c {5}\n'.format(model, self.inputFile, self.outputFile, self.dataZipFile, self.monFile, self.ascFile))
        # Execute the job script
        subprocess.call(scriptFile, cwd=self.tempDir)
    ...
    def postprocess(self, sens=False, rop=False, all=False, transpose=True):
        ...
        scriptFile = os.path.join(self.tempDir, 'RUNJOB_CKPostProcessor.bat')
        with open(scriptFile, 'w') as stream:
            stream.write(CHEMKIN_SCRIPT_PREAMBLE)
            stream.write('rem # Extract solution data to CKCSV\n')
            ...
            if transpose:
                stream.write('rem # Transpose the data to CSV\n')
                stream.write('CKSolnTranspose -column 500 {0}\n\n'.format(self.ckcsvFile))
            stream.write('rem # Delete postprocessor log file\n')
            stream.write('rem rd /s /q {0}\n\n'.format(os.path.join(self.tempDir, 'ckpp_*.log')))
